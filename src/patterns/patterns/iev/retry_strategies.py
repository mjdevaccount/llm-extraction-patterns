"""
Retry strategies for extraction phase when JSON repair fails.

Implements intelligent retry logic that refines prompts based on errors:
  - Attempt 1: Original prompt
  - Attempt 2: Refined prompt with stricter JSON instructions
  - Attempt 3: LLM-assisted repair (ask LLM to fix its own output)
  - Fallback: Return None (let validation handle it)
...
class PromptRefinementRetry(IRetryStrategy):
    ...
    async def retry(
        ...
        logger.info(f"[IEV] Extraction retry {attempt_number}: Refined prompt")
        ...
        try:
            ...
            logger.info(f"[IEV] Extraction retry {attempt_number}: Success!")
            return parsed
        
        except Exception as e:
            logger.warning(f"[IEV] Extraction retry {attempt_number} failed: {e}")
            return None
...
class LLMAssistedRepairRetry(IRetryStrategy):
    ...
    async def retry(
        ...
        logger.info(f"[IEV] Extraction repair {attempt_number}: LLM-assisted")
        ...
        try:
            ...
            logger.info(f"[IEV] Extraction repair {attempt_number}: Success!")
            return parsed
        
        except Exception as e:
            logger.warning(f"[IEV] Extraction repair {attempt_number} failed: {e}")
            return None
...
class CompositeRetryStrategy(IRetryStrategy):
    ...
    async def retry(
        ...
        logger.warning(f"[IEV] All retry strategies exhausted after {attempt_number} attempts")
        return None
